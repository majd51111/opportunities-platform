...
drop function if exists public.submit_opportunity(text, text, text, text, text, text, text[], text[], text[], text[], bigint, text);
...
create or replace function public.submit_opportunity(
  ...
  p_category_translations jsonb default '{}'::jsonb
)
returns text
language plpgsql
...
begin
  ...
  if resolved_category_id is null and normalized_category_name is not null then
    ...
    if resolved_category_id is null then
      ...
      insert into public.categories (name, slug, translations)
      values (normalized_category_name, category_slug, coalesce(p_category_translations, '{}'::jsonb))
...
grant execute on function public.submit_opportunity(text, text, text, text, text, text, text[], text[], text[], text[], bigint, text, jsonb) to authenticated;
