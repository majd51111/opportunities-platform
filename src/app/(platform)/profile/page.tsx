...
export default function ProfilePage() {
  ...
  `}</style><main dir={dir} className="mx-auto w-full max-w-5xl px-6 py-6">
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm md:col-span-2"><div className="flex flex-wrap items-center justify-end gap-3"><button type="submit" form="profile-form" disabled={!editProfile || savingProfile} className="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">{t.profilePage.saveChanges}</button>{editProfile && <button type="button" onClick={() => setEditProfile(false)} className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-medium">{t.profilePage.cancel}</button>}<button type="button" onClick={deleteAccount} className="rounded-lg border border-red-200 px-4 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50">{t.profilePage.deleteAccount}</button></div></section>
    </div>
