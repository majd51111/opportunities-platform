...
import { getLocalizedText, localizeCategory } from "@/types";
...
type Submission = {
  ...
  category?: { id: number; name: unknown }[] | null;
...
export default function AdminOpportunitiesPage() {
  ...
  const [translationTargets, setTranslationTargets] = useState<Submission[]>([]);
  ...
  async function loadSubmissions() {
    ...
    setTranslationTargets((allOpportunityData ?? []) as Submission[]);
    setLoading(false);
  ...
  function startEditing(submission: Submission) {
    ...
    setEditForm({ title: submission.title, short_description: "", description: submission.description ?? "", direct_url: submission.direct_url ?? "", earnings_text: submission.earnings_text ?? "", countries: listToText(submission.countries), devices: listToText(submission.devices), payment_methods: listToText(submission.payment_methods), requirements: listToText(submission.requirements), category_id: submission.category_id ? String(submission.category_id) : "" });
  }

  async function saveEdit(id: string | number) {
    setReviewingId(id); setMessage("");
    try {
      const { error } = await getSupabaseBrowserClient().rpc("update_pending_opportunity", { p_opportunity_id: String(id), p_short_description: null, p_title: editForm.title, p_description: editForm.description, p_direct_url: editForm.direct_url, p_earnings_text: editForm.earnings_text || null, p_category_id: editForm.category_id ? Number(editForm.category_id) : null, p_countries: textToList(editForm.countries), p_devices: textToList(editForm.devices), p_payment_methods: textToList(editForm.payment_methods), p_requirements: textToList(editForm.requirements) });
      if (error) { setMessage(`${copy.error} ${error.message}`); return; }
    } catch (error) {
    ...
    setSubmissions((current) => current.map((item) => item.id === id ? { ...item, title: editForm.title, short_description: null, description: editForm.description, direct_url: editForm.direct_url, earnings_text: editForm.earnings_text, category_id: editForm.category_id, countries: textToList(editForm.countries), devices: textToList(editForm.devices), payment_methods: textToList(editForm.payment_methods), requirements: textToList(editForm.requirements) } : item));
    setEditingId(null);
  }
  ...
  const editListFields = <div className="grid gap-4 sm:grid-cols-2">
    <label className="grid gap-1.5 text-sm font-medium text-zinc-700">
      <span>{fieldCopy.category}</span>
      <select value={editForm.category_id} onChange={(event) => setEditForm({ ...editForm, category_id: event.target.value })} className="min-h-11 rounded-lg border border-zinc-300 px-3 py-2 font-normal">
        <option value="">{fieldCopy.categoryPlaceholder}</option>
        {categories.map((category) => <option key={category.id} value={category.id}>{localizeCategory(category.name, language) ?? getLocalizedText(category.name, language, "en") ?? ""}</option>)}
      </select>
    </label>
    <label className="grid gap-1.5 text-sm font-medium text-zinc-700">
